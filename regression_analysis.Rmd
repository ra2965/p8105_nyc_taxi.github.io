...
According to the codebook from the TLC website, the fare amount is defined as "The time-and-distance fare calculated by the meter". Therefore, we hope to explore the association between fare amount and trip distance and duration.  
...
First of all, we meed to clean the dataset:
By 
```{r }
reg = pilot %>% 
  mutate(
...
The VIF, TOL and Wi columns provide the diagnostic output for variance inflation factor, tolerance and Farrar-Glauber F-test respectively. We find that 
