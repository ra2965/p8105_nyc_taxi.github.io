...
tip_plot %>% 
  mutate(do_zone = forcats::fct_reorder(do_zone, tip_percent),
       pu_zone = forcats::fct_reorder(pu_zone, tip_percent)) %>% 
  drop_na(do_zone, pu_zone) %>% 
  plot_ly(y = ~tip_percent, color = ~do_zone, type = "box", colors = "Set2") %>% 
...
tip_plot %>% 
  group_by(pu_zone) %>% 
  summarise(pu_median = median(tip_percent)) %>% 
  mutate(
  pu_zone = forcats::fct_reorder(pu_zone, pu_median)) %>% 
  ggplot(aes(x = pu_zone, y = pu_median)) +
